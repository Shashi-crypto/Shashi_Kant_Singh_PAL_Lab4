...
*    to store the data for the E-commerce with the schema definition
**/
...
*    for supplier,customer,category,product,supplier_pricing,order,rating 
*    to store the data for the E-commerce 
**/
...
Select count(ID) as CustomerCount, Gender from(Select distinct customer.CUS_ID as ID, customer.CUS_GENDER as Gender from Customer inner join `ORDER` on customer.CUS_ID = `ORDER`.CUS_ID
...
Select product.PRO_NAME as Product_Name, `ORDER`.* from `ORDER`, supplier_pricing, product where `ORDER`.CUS_ID=2 and `ORDER`.PRICING_ID= supplier_pricing.PRICING_ID
and supplier_pricing.PRO_ID= product.PRO_ID;


/**
* 5) Display the Supplier details who can supply more than 
*    one product.
**/
...
*    print the table with category id, name, product name and 
...
select supp.SUPP_ID as Supplier_ID, supp.SUPP_NAME as Supplier_Name, rat.RAT_RATSTARS as Rating,
CASE
